...
//    private final OrderItemService orderItemService;
    ...
    public Response bindPrint(SlwPrintInfo printInfo) {
        try {
            ...
            logger.error("打印机绑定异常" + e.getMessage(), e);
...
//            }
    ...
    public Response getModelContent(DaquPrintContentVo printContentVo) {
        ...
        try {
            ...
            result = DaquPrintContentUtils.getContent(printContentVo);
        } catch (Exception e) {
            logger.error("状态异常" + e.getMessage(), e);
        }
    ...
    private void syncPrint(String sn, String orderId, String content) {
        try {
            Thread thread = new Thread(() -> {
                logger.info("订单{}异步打印开始", orderId);
                String printMsg = DaquzhinengPrintUtils.print(sn, content, true);
                logger.info("订单{}异步打印返回结果：{}", orderId, printMsg);
                ...
                if (parseObject.getInteger("code") == 0) {
                    ...
                    if (dataJsonObj != null) {
                        ...
                        logger.info("打印信息：当前打印队列数量：{}", queueSize);
                        if (queueSize > 10) {
                            return;
                        }
                        // 打印请求受理ID
            ...
            logger.error("异步拉起打印" + e.getMessage(), e);
        }
    }
